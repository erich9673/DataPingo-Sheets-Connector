    ...
    try {
        ...
        if (tokenData.googleCredentials) {
            try {
                ...
                safeError('Error checking Google auth status:', error);
            }
        }
        ...
        safeError('Verify token error:', error);
        res.status(500).json({ 
            success: false, 
            error: error instanceof Error ? error.message : 'Unknown error' 
        });
...
app.post('/api/admin/monitoring-jobs/stop/:jobId', async (req, res) => {
    try {
        ...
        if (result.success) {
            safeLog(`🛑 [ADMIN] Stopped monitoring job: ${fullJobId}`);
            return res.json({ success: true });
        } else {
            safeError('Failed to stop job:', result.error);
            return res.status(500).json({ success: false, error: result.error || 'Failed to stop job' });
        }
    } catch (error) {
        safeError('Stop monitoring job error:', error);
...
app.post('/api/admin/monitoring-jobs/stop-all', async (req, res) => {
    try {
        ...
        safeLog(`🛑 [ADMIN] Stopped all monitoring jobs (${stoppedCount} total)`);
        return res.json({ success: true, stopped: stoppedCount });
    } catch (error) {
        safeError('Stop all monitoring jobs error:', error);
        return res.status(500).json({ success: false, error: error instanceof Error ? error.message : 'Unknown error' });
    }
});
